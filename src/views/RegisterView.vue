    ...
    <div class="register_view">
        ...
        <div class="form">
            <div class="input-wrapper">
                <input type="email" id="email" v-model="email" placeholder="Електронна пошта" />

                <small class="input-error">{{ error.email }}</small>
            </div>

            <div class="input-wrapper">
                <input type="text" id="phone" v-model="phone" placeholder="Номер телефону" />
            ...
            <div class="row">
                <div class="input-wrapper">
                    <input type="text" v-model="surname" id="surname" placeholder="Прізвище" />

                    <small class="input-error">{{ error.surname }}</small>
                </div>

                <div class="input-wrapper">
                    <input type="text" v-model="name" id="name" placeholder="Ім'я" />
        ...
        <div class="button-wrapper">
            <button id="submit-button" :disabled="processing" type="button" @click="send">
                Зареєструватись
            </button>
...
export default {
    ...
    mounted() {
        Inputmask({ mask: '+380 999 999 999' }).mask(document.getElementById('phone'));
    },
    methods: {
        send() {
            let isError = false;
            ...
            const isValidPhone = validator.isMobilePhone(this.phone.replaceAll(' ', ''), 'uk-UA');
            if (!isValidPhone) {
            ...
            if (!isValidPassword) {
                this.error.password = 'Пароль повинен містити принаймні 8 символів латиницею';
                isError = true;
            }
                ...
                .then((userCredential) => {
                    ...
                    updateProfile(user, {
                        ...
                        .then(() => {
                            ...
                            manager.addProfile(this.phone, user.uid).then(() => {
                                this.$router.push('/');
                            });
                        })
                        .catch((error) => {
